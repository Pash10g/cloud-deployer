...
function deploy_vm {
	...
	if [ "$machine_status" = "started" ]; then
        	...
        	eval "$2=$(juju status --format tabular | grep "^${machine_no} .*" | awk '{print $4}')"
	else
        	exit 2
	fi
}
...
#do
#	echo "shard${i} :" >> /tmp/mongo-conf.yaml
#	echo "  replicaset : <shard_repl_set_name>_${i}" >> /tmp/mongo-conf.yaml
#	echo "  port : <mongos_port>" >> /tmp/mongo-conf.yaml
...
#	echo "  replicaset : configsvr " >> /tmp/mongo-conf.yaml
#	echo "  config_server_port : <configsvr_port>" >> /tmp/mongo-conf.yaml
#	echo "  extra_daemon_options: --configsvr" >> /tmp/mongo-conf.yaml
	#juju deploy mongodb  "configsvr${i}" -n<configsvr_repl_set_number>  --constraints "mem=<configsvr_mem_mb> cpu-cores=<configsvr_cpu_core>" --config /tmp/mongo-conf.yaml || { echo "Failed to deploy mongodb configsvr '-n <configsvr_repl_set_number>' "; exit 2; }
	echo "Starting deploy configsvr${i}"
	deploy_vm "cpu-cores=<configsvr_cpu_core>" machine
...
do
	echo " Starting deploy of mongos${i}"
        deploy_vm "mem=<mongos_mem_mb> cpu-cores=<mongos_cpu_core>" machine_mongos
	fqdn=$(juju status --format tabular | grep ${machine_mongos} | awk '{print $3}')
        echo "mongos${i}: " >> /tmp/mongo-conf.yaml
        echo "  config_server_port : <mongos_port>" >> /tmp/mongo-conf.yaml
        echo "  machine: ${machine_mongos}" >> /tmp/mongo-conf.yaml
        echo "  FQDN: ${fqdn} " >> /tmp/mongo-conf.yaml
...
for i in {1..<shard_number>} 
do
#	juju deploy mongodb  "shard${i}" -n<shard_repl_set_number> --constraints "mem=<shard_mem_mb> cpu-cores=<shard_cpu_core>" --config /tmp/mongo-conf.yaml  || { echo "Failed to deploy mongodb shard${i} '-n <shard_repl_set_number>' "; exit 2; }
	...
	deploy_vm "mem=<shard_mem_mb> cpu-cores=<shard_cpu_core>" machine_shard
	fqdn=$(juju status --format tabular | grep ${machine_shard} | awk '{print $3}')
	 echo "shard${i}: " >> /tmp/mongo-conf.yaml
        echo "  config_server_port : <shard_port>" >> /tmp/mongo-conf.yaml
        echo "  machine: ${machine_shard}" >> /tmp/mongo-conf.yaml
        echo "  FQDN: ${fqdn} " >> /tmp/mongo-conf.yaml
