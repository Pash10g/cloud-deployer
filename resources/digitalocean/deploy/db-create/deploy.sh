...
function deploy_vm {
	...
	export machine_no=$(juju docean add-machine  --constraints="$constraints" 2>/dev/null | tail -n 1 | awk '{print $3}') #|| {echo "failed to init machine constraints $constraints "; exit 2}
