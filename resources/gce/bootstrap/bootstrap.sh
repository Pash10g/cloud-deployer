...
juju bootstrap -v  --constraints "cpu-cores=2 mem=3G" || { echo "ERROR While bootstraping juju env <env_name> "; exit 2; }
...
echo "Running install script for chef server on $bootstarp_node ... Can take up to 10m0s"
juju run "sudo /tmp/install_chef_server.sh https://web-dl.packagecloud.io/chef/stable/packages/ubuntu/trusty/chef-server-core_12.3.1-1_amd64.deb admin mongodb123  2>&1" --machine ${machine_no} --timeout "10m0s" || { echo "ERROR Bottstraping chef server install for <env_name> env "; exit 2; }
